...
import { BUSD, RUN, USDC } from '@pancakeswap/tokens'
...
export const useCakeBusdPrice = ({ forceMainnet } = { forceMainnet: false }): Price<Currency, Currency> | undefined => {
  ...
  // const cake: Token = isTestnet ? RUN[ChainId.BSC_TESTNET] : RUN[ChainId.BSC]
  const cake: Token = RUN[chainId]
  const defaultToken = RUN[ChainId.BSC]
  // eslint-disable-next-line react-hooks/rules-of-hooks
  return cake?.chainId ? usePriceByPairs(BUSD[cake?.chainId], cake) : usePriceByPairs(BUSD[defaultToken?.chainId], cake)
}
