        ...
        <AutoColumn gap="8px" style={{ padding: '16px' }}>
          ...
          {isStableLP ? null : (
            <FixedHeightRow>
              ...
              {token1Deposited ? (
                <RowFixed>
                  <Text ml="6px">{Number(token1Deposited?.toSignificant(6)).toLocaleString('en', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</Text>
                </RowFixed>
              ) : (
                '-'
              )}
          ...
          {userPoolBalance && JSBI.greaterThan(userPoolBalance.quotient, BIG_INT_ZERO) && (
            <Flex flexDirection="column">
              ...
              <Button
                ...
                variant="text"
                startIcon={<AddIcon color="#ff592c" />}
                width="100%"
              >
               <Text color="#ff592c" fontWeight="600"> {t('Add liquidity instead')}</Text>
              </Button>
            </Flex>
          )}
        </AutoColumn>
