...
import { Box, Flex, Text, Input, CheckmarkIcon, PencilIconFarms, IconButton } from '@pancakeswap/uikit'
...
const RoiCardInner = styled(Box)`
  ...
  padding: 24px;
  border-radius: ${({ theme }) => theme.radii.default};
  background: ${({ theme }) => theme.colors.roiatcurrent};
`
...
`
const CustomRoiInput = styled(Input)`
          ...
          {mode === CalculatorMode.PRINCIPAL_BASED_ON_ROI ? (
            ...
            <>
              <RoiDisplayContainer onClick={onEnterEditing}>
                ...
                {/* <Text fontSize="24px" bold color='white'>
                  $
                </Text> */}
                <RoiDollarAmount fontSize="24px" bold fadeOut={roiUSD > TRILLION} color='white'>
                  {roiUSD.toLocaleString('en', {
                    minimumFractionDigits: roiUSD > MILLION ? 0 : 2,
                    maximumFractionDigits: roiUSD > MILLION ? 0 : 2,
                  })}
