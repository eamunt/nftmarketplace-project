    ...
    <AutoColumn style={{ padding: '0 16px' }}>
      <RowBetween>
        ...
        <RowFixed>
          <Text fontSize="14px" color="#000000">
            {isExactIn
              ? `${Number(slippageAdjustedAmounts[Field.OUTPUT]?.toSignificant(4)).toLocaleString('en', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ${trade.outputAmount.currency.symbol}` ??
                '-'
              : `${Number(slippageAdjustedAmounts[Field.INPUT]?.toSignificant(4)).toLocaleString('en', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ${trade.inputAmount.currency.symbol}` ?? '-'}
          </Text>
        </RowFixed>
      </RowBetween>
      <RowBetween>
