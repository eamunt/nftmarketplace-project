...
export default function SwapCommitButton({
    ...
    swapCallback()
      ...
      .catch((error) => {        
        setSwapState({
          ...
          swapErrorMessage: "Swap failed: An unspecified error occurred. Please try again",
          txHash: undefined,
        })
      })
  ...
  if (showWrap) {
    return (
      <CommitButton style={{marginTop: "15px"}} width="100%" disabled={Boolean(wrapInputError)} onClick={onWrap}>
    ...
    return (
      <>
        <RowBetween>
          ...
          >
            {approval === ApprovalState.PENDING ? (
              <AutoRow gap="6px" justify="center">
                {t('Enabling')} <CircleLoader stroke="textDisabled" />
              </AutoRow>
            ) : approvalSubmitted && approval === ApprovalState.APPROVED ? (
              t('Enabled')
            ) : (
          ...
          <CommitButton
            // variant={isValid && priceImpactSeverity > 2 ? 'light' : 'primary' }
            onClick={() => {
              onSwapHandler()
            }}
            width="48%"
  ...
  return (
    <>
      <CommitButton
        // variant={isValid && priceImpactSeverity > 2 && !swapCallbackError ? 'light' : 'primary'}
        onClick={() => {
          onSwapHandler()
        }}
        id="swap-button"
