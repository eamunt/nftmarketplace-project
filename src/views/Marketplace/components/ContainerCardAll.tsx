...
import { fetchNumberPageCurrent } from 'state/marketplace/actions'
...
const ContainerCard: React.FC<Props> = ({ listItems }) => {
  ...
  // const itemsPerPage = itemOffset === 0 && isTestNet ? 8 : 9

  const [initpagenumber,setInitPageNumber] = useState(0)
  const dispatch = useDispatch<AppDispatch>()
  const valSelector =  useSelector((state: any) => state.marketplace.pagenumbermkl) || 0
  ...
  const handlePageClick = (event) => {
    ...
    setInitPageNumber(event.selected)
  }
  useEffect(() => {
