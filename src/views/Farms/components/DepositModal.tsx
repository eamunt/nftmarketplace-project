...
const DepositModal: React.FC<React.PropsWithChildren<DepositModalProps>> = ({
  ...
  function renderCurenccySymbol(){
    if ( chainId === ChainId.ONUS ) {
      return `${formattedAnnualRoi} VNDC`
    }
    return `$ ${formattedAnnualRoi}`
  }
  ...
  return (
    <CsModal title={t('Stake LP tokens')} onDismiss={onDismiss}>
      <ModalInput
        value={val}
          ...
          {Number.isFinite(annualRoiAsNumber) ? (
            ...
            >
              { chainId === ChainId.ONUS ?
                <AnnualRoiDisplay>{formattedAnnualRoi} VNDC</AnnualRoiDisplay>
              :
      ...
      <LinkExternal color='secondary100' href={addLiquidityUrl} style={{ alignSelf: 'center' }}>
