 ...
 */
export const usePriceRunBusd = ({ forceMainnet } = { forceMainnet: false }): BigNumber => {
...
export const useTokenPrice = (token) => {
  ...
  useEffect(() => {
    const fetchTokenPrice = async () =>{
      try {
        const tokenPriceObject = await getTokenPrice(token)
        setPrice(new BigNumber(tokenPriceObject?.usdPrice.toString()))
      } catch (error ) {
        console.log(error)
      }
    }
